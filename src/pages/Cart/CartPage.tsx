...
function BookCard({ book }: { book: Book }) {
  ...
  const [flash, setFlash] = useState<'digital' | 'paper' | null>(null)

  const inCartDigital = hasItem(book._id, 'digital')
  const inCartPaper   = hasItem(book._id, 'paper')

  const quickAdd = (e: React.MouseEvent, type: 'digital' | 'paper') => {
    e.preventDefault()
    e.stopPropagation()
    if (hasItem(book._id, type)) return
      ...
      <div className="w-24 min-w-24 h-32 rounded-xl flex-shrink-0 overflow-hidden shadow-md">
        {book.coverUrl ? (
          ...
          <div
            ...
            style={{
              background: `${book.collectionId?.color ?? '#6366f1'}18`,
              borderLeft: `3px solid ${book.collectionId?.color ?? '#6366f1'}`,
            }}
          >
            <p className="text-[9px] font-bold text-gray-700 leading-tight line-clamp-3">
              {book.title}
            </p>
          </div>
        )}
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-1.5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {book.isAvailableInSubscription && (
            <span className="text-[9px] px-2 py-0.5 rounded-full bg-indigo-50
                             text-indigo-500 font-semibold border border-indigo-100">
              ∞ Abonnement
            </span>
          )}
          {(inCartDigital || inCartPaper) && (
            <span className="text-[9px] px-2 py-0.5 rounded-full bg-green-50
                             text-green-600 font-semibold border border-green-100">
              ✓ Dans le panier
            </span>
          )}
        </div>
          ...
          {book.levels.map(lvl => (
            <span key={lvl}
              className="text-[10px] px-2 py-0.5 rounded-full bg-indigo-50
                         text-indigo-600 font-medium border border-indigo-100">
              {LEVEL_LABELS[lvl]}
            </span>
          ))}
          {book.pageCount && (
        ...
        <div className="flex items-center justify-between mt-auto pt-2 border-t border-gray-50">
          ...
          <div className="hidden group-hover:flex gap-1.5">
            ...
            >
              {flash === 'digital' ? '✓' : inCartDigital ? '✓' : '+ Num.'}
            </button>
            <button
              onClick={e => quickAdd(e, 'paper')}
              disabled={inCartPaper}
              className={`text-[10px] font-semibold px-2.5 py-1.5 rounded-lg transition-colors ${
                inCartPaper
                  ? 'bg-green-100 text-green-600 cursor-default'
            ...
            >
              {flash === 'paper' ? '✓' : inCartPaper ? '✓' : '+ Papier'}
            </button>
          </div>
...
function CollectionFilter({
  ...
  return (
    <div>
      ...
      <button onClick={() => onSelect('all')} className={fc(activeCol === 'all')}>
