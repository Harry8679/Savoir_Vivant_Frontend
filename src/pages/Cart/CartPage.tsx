...
export default function CartPage() {
  ...
  const { items, removeItem, updateType, clearCart, total } = useCartStore()
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-3">
          {items.map(item => (
            <div key={`${item.bookId}-${item.type}`}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex gap-1.5">
                  {(['digital', 'paper'] as CartItemType[]).map(t => (
                    <button key={t} onClick={() => updateType(item.bookId, t)}
                      className={`text-[10px] font-semibold px-2.5 py-1 rounded-full
                                  transition-colors ${
                        item.type === t
                          ? t === 'digital' ? 'bg-indigo-500 text-white' : 'bg-amber-500 text-white'
              ...
              <div className="flex flex-col items-end justify-between">
                ...
                <button onClick={() => removeItem(item.bookId, item.type)}
                  className="text-gray-300 hover:text-red-400 transition-colors text-sm mt-2">
                  🗑
                </button>
              </div>
            </div>
          ...
          <Link to="/catalogue" className="block text-sm text-indigo-500 hover:underline mt-2">
              ...
              {items.map(item => (
                <div key={`${item.bookId}-${item.type}`}
                  ...
                  <span className="line-clamp-1 flex-1 mr-2">
                    {item.title}{' '}
                    <span className={item.type === 'digital' ? 'text-indigo-400' : 'text-amber-500'}>
